#!/usr/bin/env python3
"""Scheme C (linear-in-edge sizing) with:
  - MAX bet = $2.50 (down from $3.00)
  - 2 entries per market
  - Minimum 10s gap between entries on same market
Everything else identical to debug_sizing_2x10s_max3.py.
"""
import io, json, sys
...
THRESHOLD = 0.3067
MIN_NOTIONAL = 1.00
MAX_NOTIONAL = 2.50                       # <-- the only change vs the $3 run
...
print(f"  skipped (within 10s of prior entry): {n_skipped_gap}")
...
print(f"  Calmar (total/DD):   {pnl.sum() / max(0.01, max_dd):.1f}")
...
prior_results = [
    ...
    ("C. linear max $3, 2/market w/ 10s",   606,  76.9, 1.90, 1280.99,1494.49,16.32, 78.5),
    ("Current: max $2.50, 2/market w/ 10s", n_act, wr*100, float(nots[active].mean()),
...
Path("docs/sizing_2entries_max2_50.json").write_text(json.dumps({
    "model": str(ART),
    "threshold": THRESHOLD,
...
print(f"\nSaved to docs/sizing_2entries_max2_50.json")
